    ...
    <div className="h-auto lg:h-calc-screen-minus-100 space-y-4 pb-6 px-2">
        <div className="lg:mx-10 lg:w-[80vw] flex gap-2 lg:gap-10 text-center lg:text-justify mt-20px lg-mt-1">
          ...
          <div className='flex-1 flex flex-col pt-4 lg:pt-10'>
            ...
            <p className="mt-7 lg:mt-3">{shawarma.desc}</p>
            <h3 className='font-semibold text-lg mt-5 pb-2 hidden lg:block'>Choose the size</h3>
            <div className='hidden lg:flex justify-between w-100% lg:w-40% sm:px-4 sm:py-0 lg:px-0 lg:pt-1 lg:pb-3' >
            ...
            <h3 className="hidden lg:block font-semibold text-lg">Choose additional ingredients</h3>
            ...
            <div className='container hidden lg:flex'>
              <input type='number' value={quantity} min='0' className=' sm:h-50px w-50px lg:h-10 rounded indent-2' onChange={(e) => setQuantity(e.target.value)}/>
              <button onClick={handleOrder} className={`${styles.button} bgmain bg-main-bg-600 w-full text-white font-medium cursor-pointer ml-10px px-2 text-center sm:h-50px lg:h-10 `}>Add to Cart</button>
            </div>
          </div>
        </div>
        <div className='lg:hidden pt-2'>
            <h3 className='font-semibold text-lg text-center pb-3'>Choose the size</h3>
            <div className='flex w-full justify-around lg:w-40% sm:px-4 pb-3 lg:px-0px lg:py-0px' >
              <div className='cursor-pointer relative w-30px h-30px' onClick={() => handleSize(0)}>
                <Image src="/img/size.png" layout="fill" alt="" />
                <span className='absolute text-xss text-white bg-teal rounded-md' style={{top:'-5px', right:'-20px', padding:'0 5px'}}>Small</span>
              </div>
            ...
            <h3 className="text-center font-semibold text-lg">Choose additional ingredients</h3>
            <div className='flex mt-1 mb-4 mx-2'>
              {shawarma.extraOptions.map(option => (
                <div className='flex items-center mr-1 lg:text-14px font-my500 sm:m-1' style={{marginRight:'10px'}} key={shawarma._id}>
                <input type="checkbox" id={option.text} name={option.text} className='w-25px lg:w-20px h-25px lg:h-20px' onChange={(e) => handleChange(e, option)}/>
                <label htmlFor='double' className='p-1'>{option.text}</label>
              </div>
              ))}
